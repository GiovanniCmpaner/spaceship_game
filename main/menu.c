...
static void menu_draw(uint8_t menu){
    ...
    lcd_set_font( &font20x14 );
    for(uint8_t n = 0; n < 5; n++){
        if(n == menu){
            //lcd_draw_image( 5, 15 + n * 25, &image_cursor );
        }
        lcd_printf(30,10 + n * 25,menu == n ? COLOR_WHITE : COLOR_RED, "Teste1");
    }
...
static void menu_process(){
    ...
    while(1){
        const uint8_t menuAnterior = menuAtual;
        for(size_t n = 0; n < 4; n++){
            ...
            if(input_action(n,&level) && level){
                if(n == ACTION_UP){
                    ...
                    game_initialize();
                    menu_terminate();
                    //audio_play_embed_wav("/sdcard/sample2.wav);
                }
                else if(n == ACTION_RIGHT){
                    network_initialize( NETWORK_CLIENT, NETWORK_TCP );
                    //audio_play_embed_wav(cursor_ley.wav);
                }
